...
class RRT(PathPlanner):
    ...
    def start_new_plan(self, 
        ...
        ## FIXME: tmp added for ActiveGS ##
        self.free_xs, self.free_ys, self.free_zs = np.where(self.sdf_map > 1)

        self.rrt_iter = 0

    def generate_random_point(self, full_range: bool = False, use_free_space: bool = False) -> np.ndarray:
        ...
        if full_range:
            ...
            rand_idx = np.random.randint(len(self.free_xs))
            x, y, z = self.free_xs[rand_idx], self.free_ys[rand_idx], self.free_zs[rand_idx]
        else:
